...
Status PrintTopLabels(const std::vector<Tensor>& outputs,
                      const string& labels_file_name, std::string file_path, std::ofstream& outfile) {
  ...
  for (int pos = 0; pos < how_many_labels; ++pos) 
  {
    ...
    //For txt file output
    outfile <<file_path<<" => "<< labels[label_index] << " (" << label_index << "): " << score<<"\n";
  }
  outfile <<"\n";
  return Status::OK();
}
...
int main(int argc, char* argv[]) {
  ...
  //Additional Code : Extract all jpg files from directory
  std::vector<std::string> image_list;
  std::string filePath = "tensorflow/examples/label_image/data/";
  DIR *dir;
  struct dirent *ent;
  if ((dir = opendir (filePath.c_str())) != NULL) 
  {
    ...
    {
      if(string(ent->d_name).substr(string(ent->d_name).find_last_of(".") +1) == "jpg")
        image_list.push_back(filePath + string(ent->d_name));
    }
    closedir (dir);
  }
  //Print final result to txt file
  std::ofstream outfile("tensorflow/examples/label_image/data/Inference_Result.txt", std::ofstream::out);

  string image = "tensorflow/examples/label_image/data/sample.jpg";
  string graph =
      "tensorflow/examples/label_image/data/output_graph.pb";
  string labels =
      "tensorflow/examples/label_image/data/output_labels.txt";
  ...
  for(int i=0; i<image_list.size();i++)
  {
    ...
    string image_path = tensorflow::io::JoinPath(root_dir, image_list[i]);
    ...
    Status print_status = PrintTopLabels(outputs, labels, image_list[i], outfile);
    if (!print_status.ok()) {
